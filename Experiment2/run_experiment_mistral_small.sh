...
# Settings
data="data_cleaned"  # name of the input data (without .jsonl extension)
gen_model="mistral-small"
filter_model="mistral-small"
eval_model="qwen2.5:14b"
#
model_name="mistral_small" ## needed since outputed files from below scripts will replace - and : with _ in the model name
...
python3 $code_vanilla       $gen_model  "data/$data.jsonl"   ./responses_Vanilla
python3 $code_controlled    $gen_model  "data/$data.jsonl"   ./responses_Basic     $topics
python3 $code_controlled    $gen_model  "data/$data.jsonl"   ./responses_Extended  $topics
...
python3 $code_filter        $filter_model "responses_Basic/$data.$model_name.jsonl"        ./responses_Basic        $topics              $filter_prompt
python3 $code_filter        $filter_model "responses_Extended/$data.$model_name.jsonl"     ./responses_Extended     $topics              $filter_prompt
